...
# variables
	...
	?>
		...
		@foreach(${str_replace(".limit(500)", "", $status)} as $user)
			...
			if (is_null($row)) {
				echo "added ";
				//echo $user['id'] . ", " . $user['rsvp_status'] . ", " . $user['name'] ;
				$row = new fb_rsvp;
			}
			...
			//$list[] = array('id'=>$row->id, 'fb_eventid'=>'$event_id', 'fb_userid'=>$user['id'], 'rsvp_status'=>$user['rsvp_status'], 'fb_name'=>$user['name'], 'updated_at'=>NULL);
...
?>
<p>All Completed :)<br /><a href="fb_events">Return</a>
@endsection